...
class TCCCalculation extends MetricCalculator_1.MetricCalculator {
    calculate(node, FECFC, Filename) {
        ...
        if (fileParsedComponents) {
            ...
            console.log(`[TCC] Found ${classGroups.length} class groups in ${Filename}`);
            classGroups.forEach((classGroup, index) => {
                console.log(`[TCC] Processing class group #${index + 1}: ${classGroup.name || "unnamed"}`);
                console.log(`[TCC] Adding ${classGroup.methods.length} methods and ${classGroup.fields.length} fields`);
        ...
        console.log(`[TCC] Total methods: ${allMethods.length}, Total fields: ${allFields.length}`);
        console.log(`[TCC] Field names: ${allFields.map((f) => f.name).join(", ")}`);
    ...
    calculateTCC(methods, fields) {
        ...
        console.log(`[TCC] Filtered out constructors. Relevant methods: ${relevantMethods.length}`);
        console.log(`[TCC] Method names: ${relevantMethods.map((m) => m.name).join(", ")}`);
        const numMethods = relevantMethods.length;
        // If there are 0 or 1 methods, return that number as the TCC
        if (numMethods <= 1) {
            console.log(`[TCC] Not enough methods (${numMethods}). Returning ${numMethods}`);
            return numMethods;
        }
        let cohesivePairs = 0;
        console.log(`[TCC] Calculating cohesive pairs for ${numMethods} methods...`);
        ...
        for (let i = 0; i < relevantMethods.length; i++) {
            ...
            console.log(`[TCC] Fields used by ${methodA.name}: ${methodA.fieldsUsed.join(", ") || "none"}`);
            // Create a set of methods that access the same fields as methodA
            const methodsAccessingSameFields = new Set();
            // For each field used by methodA
            for (const fieldName of methodA.fieldsUsed) {
                console.log(`[TCC] Checking field "${fieldName}" used by ${methodA.name}`);
                ...
                for (const methodB of relevantMethods) {
                    if (methodB !== methodA && methodB.fieldsUsed.includes(fieldName)) {
                        ...
                        console.log(`[TCC] Found method ${methodB.name} also using field "${fieldName}"`);
                    }
                }
            }
            console.log(`[TCC] Method ${methodA.name} shares field access with ${methodsAccessingSameFields.size} other methods: ${Array.from(methodsAccessingSameFields).join(", ")}`);
