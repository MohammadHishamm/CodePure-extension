...
export class JavaDataAbstractionCoupling extends MetricCalculator {
  ...
  private findDataAbstractionCoupling(Fields: FieldInfo[]): number {
    ...
    const primitiveTypes = new Set(
      primitiveTypesList.map((type) => type.toLowerCase())
    );

    console.log(
      "[DAC] Primitive types that will be ignored:",
      primitiveTypesList
    );
    ...
    for (const field of Fields) {
      ...
      console.log(
        `[DAC] Field: ${field.name || "unnamed"}, Type: ${
          fieldType || "undefined"
        }`
      );
      ...
      if (genericMatch) {
        ...
        console.log(
          `[DAC] Container: ${genericMatch[1]}, Element: ${genericMatch[2]}`
        );
        console.log(
          `[DAC] Note: Current implementation skips processing generic types`
        );
      } else {
        ...
        if (!primitiveTypes.has(fieldType.toLowerCase())) {
          if (!usedClassTypes.has(fieldType)) {
            ...
            DAC++;
            console.log(
              `[DAC] ✅ COUNTED as DAC: ${fieldType} (new non-primitive type)`
            );
          } else {
